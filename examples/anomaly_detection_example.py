...
async def main() -> None:
    ...
    for data_set_date, size in ((20260312, 100), (20260313, 101), (20260314, 103)):
        historical_context = AnalyzerContext()
        historical_context.store_metric(AnalyzerMetric("size", "size", size))
        repository.save(
            ResultKey(data_set_date=data_set_date, tags={"dataset": "users"}),
            historical_context,
        )
    ...
    for anomaly in anomalies:
        print(
            f"  - {anomaly.metric_key}: current={anomaly.current_value}, "
            f"expected={anomaly.expected_value}, strategy={anomaly.strategy_name}, "
